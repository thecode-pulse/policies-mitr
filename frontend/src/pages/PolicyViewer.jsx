...
import { FiArrowLeft, FiClock, FiTag, FiBarChart2, FiMessageCircle, FiSquare, FiVolume2, FiGlobe, FiChevronUp, FiChevronDown, FiSend, FiCopy, FiDownloadCloud } from 'react-icons/fi';
...
export default function PolicyViewer() {
    ...
    const handleDownloadPDF = () => {
        toast.success("Generating clean PDF...", { icon: '📄' });

        let translationHTML = '';
        if (translatedSummary && activeTranslatedLang) {
            translationHTML = `
                <div style="margin-top: 35px; page-break-inside: avoid;">
                    <h2 style="font-size: 18px; font-weight: 700; color: #111; border-bottom: 2px solid #8b5cf6; padding-bottom: 8px; margin-bottom: 16px; display: inline-block;">
                        Translated Summary (${activeTranslatedLang.name})
                    </h2>
                    <p style="font-size: 14px; line-height: 1.8; color: #333; white-space: pre-wrap; margin: 0;">${translatedSummary}</p>
                </div>
            `;
        }

        const htmlContent = `
            <div style="width: 800px; padding: 40px 50px; font-family: 'Inter', system-ui, -apple-system, sans-serif; color: #000; background-color: #fff;">
                <div style="margin-bottom: 30px; border-bottom: 2px solid #e2e8f0; padding-bottom: 24px;">
                    <div style="color: #64748b; font-size: 11px; font-weight: 800; text-transform: uppercase; letter-spacing: 1px; margin-bottom: 12px;">
                        ${policy?.category || 'Policy Document'}
                    </div>
                    <h1 style="font-size: 28px; font-weight: 900; color: #0f172a; margin: 0; line-height: 1.3;">
                        ${policy?.title || 'Policy Document'}
                    </h1>
                </div>
                
                <div style="margin-top: 20px;">
                    <h2 style="font-size: 18px; font-weight: 700; color: #111; border-bottom: 2px solid #0ea5e9; padding-bottom: 8px; margin-bottom: 16px; display: inline-block;">
                        Executive Summary
                    </h2>
                    <p style="font-size: 14px; line-height: 1.8; color: #333; white-space: pre-wrap; margin: 0;">${policy?.summary || ''}</p>
                </div>
                
                ${translationHTML}
                
                <div style="margin-top: 35px; page-break-inside: avoid;">
                    <h2 style="font-size: 18px; font-weight: 700; color: #111; border-bottom: 2px solid #10b981; padding-bottom: 8px; margin-bottom: 16px; display: inline-block;">
                        Simplified Insights
                    </h2>
                    <div style="background-color: #f8fafc; padding: 20px; border-radius: 8px; border: 1px solid #e2e8f0;">
                        <p style="font-size: 14px; line-height: 1.8; color: #333; margin: 0;">${policy?.simplified || 'No simplified insights generated.'}</p>
                    </div>
                </div>
                
                <div style="margin-top: 50px; padding-top: 20px; border-top: 1px solid #e2e8f0; text-align: center; color: #94a3b8; font-size: 11px; font-weight: 600;">
                    Generated by PolicyMitr AI Assistant
                </div>
            </div>
        `;

        const safeTitle = (policy?.title || 'policy_document').slice(0, 30).replace(/[^a-z0-9]/gi, '_').toLowerCase();
        const opt = {
            margin: 15,
        ...
        <div style={{ maxWidth: '1000px', margin: '0 auto', paddingBottom: '40px' }}>
            <motion.div initial="hidden" animate="visible" variants={containerVariants}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
                    ...
                    <motion.div variants={itemVariants} style={{ display: 'flex', gap: '12px' }}>
                        <button
                            ...
                            style={{ padding: '8px 16px', fontSize: '13px', borderRadius: '100px', gap: '8px' }}
                        >
                ...
                <AnimatePresence mode="wait">
                    ...
                    >
                        {activTab === 'summary' && (
                            <div id="pdf-export-content" style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
                                {/* Main Summary */}
                                <div className="glass-card" style={{ padding: '32px' }}>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
                                        ...
                                        <div className="summary-actions" style={{ display: 'flex', gap: '12px', flexWrap: 'wrap', justifyContent: 'flex-end' }}>
                                            <button
                                                type="button"
                                                onClick={() => handleCopy(policy.summary)}
                                                className="btn-secondary"
                                                style={{ padding: '10px 14px', fontSize: '13px', gap: '6px' }}
                                                title="Copy Summary"
                                            >
                                                <FiCopy size={16} /> Copy
                                            </button>
                                                    ...
                                                    <div style={{ display: 'flex', gap: '8px' }}>
                                                        <button
                                                            ...
                                                            style={{
                                                                ...
                                                                color: 'var(--text-secondary)', padding: '6px 14px', borderRadius: '8px', cursor: 'pointer',
